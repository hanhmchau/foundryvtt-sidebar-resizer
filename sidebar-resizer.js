...
function _getImportantStr() {
  if (
    game.modules.get("dnd-ui")?.active ||
    game.modules.get("pathfinder-ui-legacy")?.active
  )
    return " !important";
  else return "";
}
...
function _createTinyMceChat(selector) {
  ...
  tinyMCE.init({
    ...
    plugins: "lists",
    toolbar:
      "bold italic underline | backcolor forecolor removeformat | numlist bullist | p h1 h2 h3",
    height: "100%",
      ...
      editor.on("keydown", function (event) {
        if (modKey === "enter" && event.keyCode === 13 && event.shiftKey) {
          ...
          ((modKey === "ctrl" && event.ctrlKey) ||
            (modKey === "shift" && event.shiftKey) ||
            (modKey === "enter" && !event.ctrlKey))
        ) {
...
function _assignVerticalResizer(chatform) {
  ...
  else resizer.style.position = "fixed";
  ...
  function stopResize(e) {
    window.localStorage.setItem(
      "chatform-resizer-init-size",
      chatform.offsetHeight,
    );
    window.removeEventListener("mousemove", resize, false);
    window.removeEventListener("mouseup", stopResize, false);
  }
  ...
  if (game.modules.get("lib-wrapper")?.active) {
    libWrapper.register(
      ...
      function (wrapped, ...args) {
        ...
        result.height =
          parseInt($("#board").css("height")) / _sdbFloatingDefaultSize;
        const lastSidebarSize = window.localStorage.getItem(
          "sidebar-resizer-init-size",
        );
        if (lastSidebarSize && Number.isInteger(+lastSidebarSize))
          result.width = parseInt(lastSidebarSize);
        return result;
      },
      "WRAPPER",
    );
...
Hooks.once("renderSidebarTab", function () {
  ...
  if (Number.isInteger(+lastSidebarSize)) {
    ...
    sidebar.setAttribute(
      "style",
      `width: ${lastSidebarSize}px${_getImportantStr()}`,
    );
  }
});
...
Hooks.on("renderChatLog", function (chat, div) {
  ...
  const lastChatformSize = window.localStorage.getItem(
    "chatform-resizer-init-size",
  );
